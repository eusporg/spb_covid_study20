...
for(scenario in regression_scenarios) {
	...
	# Debug: scenario <- "basic"; serotest <- "IgG_or_M_testD"
	...
	for(serotest in serological_tests) {
		...
		for(group_var in grouping_vars) {
			...
			for( surveyweight in c("no", "raking") ) {
				...
				# Debug: scenario <- "basic"; serotest <- "IgG_or_M_testD"; group_var <- "education_level"
...
for(w in unique(naive_prevalence_by_draw_sample_week_vector$draw_week)) {
	...
	for( adjusted in c(T, F) ) {
		...
		naive_prevalence_by_draw_sample_week_adjusted <- rbind(naive_prevalence_by_draw_sample_week_adjusted, out_vector, out_genetico)
...
save(prevalence_by_variable_level, file = "estimates/wave3/prevalence_by_variable_level.rdata", compress = "gzip")
