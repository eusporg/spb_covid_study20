...
infection_regressors <- "lives_alone + washing_hands_more + was_sick + selftested_covid + district"
...
serological_tests <- c("IgG_or_M_testD", "IgA_or_G_or_M_testC", "testC_and_testD", "testC_or_testD")
...
# Set test characteristics (after own test validation)
## Genetico CoronaPass (after own test validation)
sensitivity_iga_g_m_genetico <- 0.920
specificity_iga_g_m_genetico <- 1

## Vector-best (after own test validation)
sensitivity_igg_m_vector <- 0.939
specificity_igg_m_vector <- 1

## Another option: assume unitary sensitivity and specificity
#sensitivity_igg_m_vector <- specificity_igg_m_vector <- sensitivity_iga_g_m_genetico <- specificity_iga_g_m_genetico <- 1
	...
	for(serotest in serological_tests) {
		...
		for(prevalence_type in prevalence_types) {
			...
			for( surveyweight in c("no", "raking") ) {
				...
				test_sensitivity <- ifelse(grepl("testB", serotest), sensitivity_igg_abbott, ifelse(grepl("testC", serotest), sensitivity_iga_g_m_genetico, ifelse(grepl("testD", serotest), sensitivity_igg_m_vector, 1)))
				test_specificity <- ifelse(grepl("testB", serotest), specificity_igg_abbott, ifelse(grepl("testC", serotest), specificity_iga_g_m_genetico, ifelse(grepl("testD", serotest), specificity_igg_m_vector, 1)))
...
# Save point
save(prevalence_by_model, file = "estimates/wave3/prevalence_by_model.rdata", compress = "xz")
