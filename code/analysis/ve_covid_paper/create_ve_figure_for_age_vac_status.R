...
h_model_gam2 <- mgcv::gam(hospitalization ~ s(age, by = vaccination) + 
														# sex +
														# source +
														vaccination, 
													data = trc_dt, 
...
# Plot 
figure_1 <- ggplot() +  
	geom_line(data = age_status_ve_data, aes(x = age, y = hospitalization), size = 0.7)  +  
	geom_ribbon(data = age_status_ve_data, aes(x = age, ymin = CI_lower, ymax = CI_upper), alpha = 0.2) + 
